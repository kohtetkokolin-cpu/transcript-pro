...
const App: React.FC = () => {
  ...
  const handleOpenKey = async () => {
    if (window.aistudio && typeof window.aistudio.openSelectKey === 'function') {
      ...
      // Optimization: trigger a check immediately after returning from dialog
      const selected = await window.aistudio.hasSelectedApiKey();
      setHasApiKey(selected);
    }
  };
      ...
      <header className="sticky top-0 z-40 w-full bg-white/80 backdrop-blur-xl border-b border-slate-200/60 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 h-24 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-bold text-slate-500">
            <button onClick={() => setCurrentView('dashboard')} className="hover:text-indigo-600 transition-colors">Tools</button>
            <button onClick={() => setCurrentView('archive')} className="hover:text-indigo-600 transition-colors">Archive</button>
            
            <div className="flex items-center gap-3">
              {hasApiKey && (
                <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg text-[9px] font-black uppercase tracking-widest border border-emerald-100">
              ...
              <button 
                ...
                className={`px-5 py-2.5 rounded-xl transition-all shadow-lg flex items-center gap-2 text-[10px] font-black uppercase tracking-widest ${hasApiKey ? 'bg-white border border-slate-200 text-slate-600 hover:border-indigo-500 hover:text-indigo-600' : 'bg-amber-500 text-white hover:bg-amber-600 shadow-amber-100 animate-pulse'}`}
              >
            ...
            <button className="px-6 py-3 bg-slate-900 text-white rounded-xl hover:bg-slate-800 transition-all shadow-lg shadow-slate-200 active:scale-95">Get Pro</button>
          </nav>
        </div>
      </header>
      <main className="max-w-7xl mx-auto px-6 py-12">{renderView()}</main>
      <footer className="max-w-7xl mx-auto px-6 py-12 border-t border-slate-200 mt-12 text-center text-sm text-slate-400 font-medium">© 2024 Transcript Pro. Powered by Gemini 3 Flash.</footer>
    </div>
  );
};
